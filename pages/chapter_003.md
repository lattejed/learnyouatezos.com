...
What's more, we can easily derive a public key from our private key, but it's computationally infeasable to derive our private key from our public key. Mathmatically, it involes prime numbers and is a type of one-way or trapdoor function. 
...
Public Key Cryptography is used extensively in blockchain, with it's most fundamental use being the derivation of keypairs for crypto wallets. A public key, in essense, becomes an address that coins can be sent to and the matching private key is the secret needed to send money *out of* that address.
...
Additionlly, each node generally holds a complete copy of the list of transacitons that the blockchain contains. 
...
This is where Game Theory comes into play. This problem is usually refered to as the Byzantine Generals Problem, which is based on a related (but simpler) problem known as the Two Generals Problem.
